...
int get_reg_info(char *reg_buf, char *user, char *nick_name, char *pwd, char *tel, char *email)
{
    ...
    */
    ...
    if(ret!=0)
    {
        LOG(REG_LOG_PROC,REG_LOG_PROC,"encode reg_username fail\n");
        goto END;
    }
    ...
    strcpy(temppwd, child3->valuestring); //拷贝内容
    //对密码进行加密
    ret = encode_message(temppwd,pwd);
    if(ret!=0)
    {
        LOG(REG_LOG_MODULE,REG_LOG_PROC,"encode reg_pwd fail\n");
    }

    //电话
    cJSON *child4 = cJSON_GetObjectItem(root, "phonenum");
    if(NULL == child4)
    {
        LOG(REG_LOG_MODULE, REG_LOG_PROC, "cJSON_GetObjectItem err\n");
        ret = -1;
    ...
    //邮箱
    cJSON *child5 = cJSON_GetObjectItem(root, "emailaddress");
    if(NULL == child5)
    {
        LOG(REG_LOG_MODULE, REG_LOG_PROC, "cJSON_GetObjectItem err\n");
        ret = -1;
    ...
    while (FCGI_Accept() >= 0)
    {
        ...
        else //获取登陆用户信息
        {
            ...
            */
            ...
            {
                printf(out); //给前端反馈信息
			    LOG(REG_LOG_MODULE, REG_LOG_PROC, "out:%s\n",out);
                free(out); //记得释放
            }else
			{
				LOG(REG_LOG_MODULE, REG_LOG_PROC, "out==NULL\n");
			}
        }
